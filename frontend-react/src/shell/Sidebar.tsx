...
export default function Sidebar({ collapsed, onToggleCollapse, onToggleTheme, badges }: Props) {
  ...
  const navCls = (path: string, pageId: string) => {
    ...
    const base = 'nav-item relative group flex items-center gap-3 px-3 py-2 rounded-lg text-sm font-medium transition-colors cursor-pointer';
  ...
  return (
    <>
      ...
      >
        {/* Logo */}
        <div className="flex items-center gap-3 px-5 h-16 border-b border-slate-200 dark:border-white/[0.04] flex-shrink-0">
        ...
        {/* Navigation */}
        <nav className="flex-1 py-1.5 px-3 overflow-y-auto">
          {groups.map((group, gi) => (
            <div key={group}>
              {gi > 0 && <div className="my-1.5 border-t border-slate-200 dark:border-slate-700/50" />}
              {!collapsed && (
                <p className="nav-group-label text-xs uppercase tracking-widest text-slate-400 dark:text-slate-500 px-3 mt-2 mb-0.5">
              ...
              {ROUTES.filter(r => r.group === group).map(route => (
                ...
                >
                  <i className={`ph-duotone ${route.icon} text-lg flex-shrink-0`} />
        ...
        {/* Bottom utility row */}
        <div className="flex items-center justify-center gap-2 py-3 border-t border-slate-200 dark:border-slate-700/50 flex-shrink-0">
          <button onClick={onToggleTheme} title="Toggle Theme" className="sidebar-util-btn w-11 h-11 rounded-lg flex items-center justify-center text-slate-500 dark:text-slate-400 hover:bg-slate-100 dark:hover:bg-slate-800 transition-colors">
            <i className="ph-duotone ph-sun text-lg hidden dark:inline-block" />
            <i className="ph-duotone ph-moon text-lg block dark:hidden" />
          </button>
          <button onClick={onToggleCollapse} title="Collapse" className="sidebar-util-btn w-11 h-11 rounded-lg flex items-center justify-center text-slate-500 dark:text-slate-400 hover:bg-slate-100 dark:hover:bg-slate-800 transition-colors">
            <i className={`ph-duotone ph-caret-double-left text-lg transition-transform ${collapsed ? 'rotate-180' : ''}`} />
          </button>
        </div>
      </aside>
