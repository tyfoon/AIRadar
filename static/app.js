...
async function refreshPage(page) {
  // All category pages (summary, dashboard, ai, cloud, privacy, iot,
  // content/family/other, geo, ips) are React — their own React Query
  // clients refetch on mount/focus so nothing for vanilla to trigger
  // here. Only Rules + Settings are thin-shells that still need the
  // vanilla refresh path; devices also reloads deviceMap so other
  // pages' bridges stay fresh.
  try {
...
// Summary / Dashboard / AI / Cloud / Privacy / Content / IoT / Geo / IPS
// are all React pages now. Rules + Settings are thin-shells that still
// use vanilla refresh code further down.

// IP/Domain Reputation — now handled by React (shared/ReputationModal.tsx,
// shared/ReputationBadge.tsx, shared/reputationApi.ts). All vanilla consumers
// (device drawer connections, country drawer IP list, settings tab) have
// been migrated.
...
// Dashboard / IoT / Geo refresh stubs removed — nothing calls them.
...
// Device detail drawer — fully migrated to React (DeviceDrawer.tsx).
// window.openDeviceDrawer is installed by AppShell as a router bridge
// that navigates to /devices?mac=<mac>. The vanilla drawer used to
// live here (~1000 lines: openDeviceDrawer, _loadDrawerActivity,
// _renderReportHTML, renderSimpleMarkdown, closeDeviceDrawer, ...)
// and all its helpers, plus the window exports for onclick handlers
// in the vanilla DeviceMatrix rendering that also went to React.
window._toggleDevGroup = _toggleDevGroup;

// _fmtDuration survives because the Rules page is a thin React shell
// that delegates to vanilla loadActiveBlockRules() — which renders
// "expires in X" badges for active rules.
...
// Safety stubs — a handful of onclick strings in other dead vanilla
// rendering paths (_renderDeviceMatrix etc.) still mention these
// names. The parent functions are never invoked from React, but JS
// will throw ReferenceError if these are called from an HTML string
// that somehow reaches the DOM. Keep them as no-ops.
function closeDeviceDrawer() { /* React drawer closes itself */ }
function renderSimpleMarkdown(md) { return md ? String(md) : ''; }
...
async function adminCleanupRun() {
  ...
  try {
    ...
    // Privacy/IPS are React pages — they refetch via React Query on
    // next focus/mount, nothing to trigger from here.
  } catch (err) {
    if (out) {
...
async function refreshIps() { /* React handles this */ }
async function loadIpsStatus() { /* React handles this */ }
...
// Expose refresh functions for the thin-shell React pages that
// delegate to vanilla. Summary / AI / Cloud / Privacy / Content /
// IoT / Dashboard / Geo / Devices / IPS are all pure React now;
// only Rules is still a thin shell that needs refreshRules.
