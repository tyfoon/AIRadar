...
// ================================================================
// IoT DEVICES — now handled by React island (react-iot-root)
// ================================================================
async function refreshIot() {
  const el = document.getElementById('react-iot-root');
  if (el && !el.getAttribute('data-active')) {
    el.setAttribute('data-active', 'true');
  }
}

// Legacy stubs so any remaining onclick handlers don't throw
window.switchIotTab = function() {};
window._deleteIotAnomaly = function() {};
window._refreshNetworkGraph = function() {};


/* --- REMOVED: classic IoT code (switchIotTab, refreshIot, _renderIotStats,
   _renderIotAnomalies, _renderIotFleet, _loadIotSparklines,
   _refreshNetworkGraph, _deleteIotAnomaly) — replaced by React island --- */
