...
function buildGrid(events: DashEvent[], isMobile: boolean) {
  ...
  events.forEach(e => {
    ...
    devTotals[d] = (devTotals[d] || 0) + (e.bytes_transferred || 1);
  });
  ...
  // Build grid: device×hour → { bytes, categoryBytes }
  const grid: Record<string, { bytes: number; catBytes: Record<string, number> }> = {};
  ...
  events.forEach(e => {
    ...
    if (!grid[key]) grid[key] = { bytes: 0, catBytes: {} };
    grid[key].bytes += bytes;
    grid[key].catBytes[cat] = (grid[key].catBytes[cat] || 0) + bytes;
  });

  // Find max bytes for normalization
  let maxBytes = 0;
  Object.values(grid).forEach(v => { if (v.bytes > maxBytes) maxBytes = v.bytes; });
  ...
  topDevs.forEach(dev => {
    for (let h = 0; h < 24; h++) {
      ...
      if (!data || data.bytes === 0) {
        cells.push({ device: dev, hour: h, bytes: 0, dominantCategory: 'none' });
      } else {
        // Find dominant category
        let topCat = 'other';
        let topCatBytes = 0;
        Object.entries(data.catBytes).forEach(([cat, b]) => {
          if (b > topCatBytes) { topCat = cat; topCatBytes = b; }
        });
        cells.push({ device: dev, hour: h, bytes: data.bytes, dominantCategory: topCat });
      }
    }
  });

  return { cells, devices: topDevs, maxBytes };
}
...
function Scene({ cells, devices, maxBytes, isMobile }: {
  cells: Cell[];
  devices: string[];
  maxBytes: number;
  isMobile: boolean;
  ...
  return (
    <>
      ...
      {cells.map((cell, i) => {
        ...
        const normalizedHeight = (cell.bytes / maxBytes) * maxHeight;
        const color = cell.bytes > 0 ? categoryColor(cell.dominantCategory) : '#334155';
        const opacity = cell.bytes > 0 ? 0.85 : 0.15;
      ...
      <div style={{ height: chartH, borderRadius: 8, overflow: 'hidden' }}>
        ...
        >
          <Scene
            ...
            maxBytes={gridData.maxBytes}
            isMobile={isMobile}
          />
        </Canvas>
      </div>
      <p className="text-[10px] text-slate-400 mt-1.5">
        X: devices &middot; Z: hour of day &middot; Height: traffic volume &middot; Drag to rotate
      </p>
    </div>
  );
}
