...
export default function GeoMap({ initialDirection = 'outbound' }: Props) {
  ...
  const { data, isLoading, isError } = useQuery({
    queryKey: ['geo-traffic', direction, period],
    queryFn: () => fetchGeoTraffic(direction, period),
    staleTime: 30_000,
  });
  ...
  useEffect(() => {
    ...
    renderMap(mapRef.current, countries, direction);
    return () => {
  ...
  function renderMap(el: HTMLDivElement, countries: GeoCountry[], _dir: Direction) {
    // Destroy previous
    if (mapInstance.current) {
      try { mapInstance.current.destroy(); } catch {}
      mapInstance.current = null;
    ...
    try {
      mapInstance.current = new jsVectorMap({
        ...
        onRegionTooltip(_event: any, tooltip: any, code: string) {
          const c = countries.find(x => x.country_code === code);
          if (c) {
            tooltip.text(
              `<b>${countryName(code)}</b><br/>` +
              `${formatBytes(c.bytes)} · ${formatNumber(c.hits)} connections`,
              true,
            );
          }
        },
        onRegionClick(_event: any, code: string) {
          // Delegate to vanilla JS country drawer
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="flex items-center gap-2">
        <div className="flex items-center gap-1 bg-slate-100 dark:bg-white/[0.04] rounded-lg p-1">
          <TabButton
            active={direction === 'outbound'}
            onClick={() => setDirection('outbound')}
            icon="ph-duotone ph-arrow-up-right"
            label="Outbound"
          />
          <TabButton
            active={direction === 'inbound'}
            onClick={() => setDirection('inbound')}
            icon="ph-duotone ph-arrow-down-left"
            label="Inbound"
          />
        </div>
        <select
          ...
          className="ml-auto text-xs border border-slate-200 dark:border-white/[0.1] rounded-md px-2 py-1.5 bg-white dark:bg-white/[0.05] text-slate-600 dark:text-slate-300"
        >
      ...
      {!isLoading && !isError && countries.length === 0 && (
        <div className="py-12 text-center text-sm text-slate-400">
          No geo traffic data for this period.
        </div>
      )}

      {/* Countries table */}
      {countries.length > 0 && (
        <CountriesTable countries={countries} direction={direction} />
      )}
    </div>
  );
}
...
function StatCard({
  ...
  return (
    <div className="bg-white dark:bg-white/[0.03] border border-slate-200 dark:border-white/[0.05] rounded-xl p-4">
      ...
      <p className="text-xl font-bold mt-1 tabular-nums text-slate-700 dark:text-slate-100">
        {value}
      </p>
      ...
      <table className="w-full text-sm">
        <thead>
          <tr className="text-[11px] text-slate-400 dark:text-slate-500 border-b border-slate-100 dark:border-white/[0.04]">
            ...
            <th className="py-2.5 px-4 text-left font-medium hidden lg:table-cell" style={{ width: '30%' }}>
              Distribution
            </th>
          </tr>
        </thead>
        <tbody>
          {countries.map((c, i) => {
            ...
            return (
              <tr
                key={c.country_code}
                className="border-b border-slate-50 dark:border-white/[0.02] hover:bg-slate-50 dark:hover:bg-white/[0.02] cursor-pointer transition-colors"
                onClick={() => {
                  if (typeof (window as any).openCountryDrawer === 'function') {
              ...
              >
                ...
                <td className="py-2.5 px-4 text-right tabular-nums text-slate-600 dark:text-slate-300">
                  {formatBytes(c.bytes)}
                </td>
                <td className="py-2.5 px-4 text-right tabular-nums text-slate-400 hidden sm:table-cell">
                  {formatNumber(c.hits)}
                </td>
